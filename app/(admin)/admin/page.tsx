  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="Total Policies" value={String(activePolicies)} rawValue={activePolicies} change="+11% this quarter" icon={<FileText className="w-5 h-5 text-electric-400" />} />
        <KPICard label="Annual Premium" value={`$${(totalPremium / 1000).toFixed(0)}K`} rawValue={totalPremium} change="+18% YoY" icon={<DollarSign className="w-5 h-5 text-electric-400" />} />
        <KPICard label="Active Agents" value={String(activeAgents)} rawValue={activeAgents} change="+2 this month" icon={<Users className="w-5 h-5 text-electric-400" />} />
        <KPICard label="Pending Applications" value={String(pendingApps)} rawValue={pendingApps} change={`${pendingApps} awaiting review`} positive={false} icon={<Clock className="w-5 h-5 text-electric-400" />} />
      </div>
      ...
      <div className="theme-card rounded-2xl p-6">
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={analyticsData.monthlyPolicies} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="policies" stroke="#3B82F6" strokeWidth={2} dot={{ fill: '#3B82F6', r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
